document.addEventListener('DOMContentLoaded', () => {
    ...
    function adjustTimer(timerId, amount) {
        ...
        if (timer.startTime !== null) {
            ...
            if (elapsedSinceOriginalStart < (110 * 1000) + timer.adjustment * 1000) {
                scheduleInitialBeeps(timerId);
            } else {
                scheduleRepeatingBeeps(timerId, elapsedSinceOriginalStart);
            }
        }
        // 誤差調整後に表示を即時更新
        updateNextBeepTime(timer.nextBeepElement, '', timerId);
    ...
    function scheduleRepeatingBeeps(timerId, elapsedSinceOriginalStart) {
        const timer = timers[timerId];
        ...
        const currentCycleElapsed = (elapsedSinceOriginalStart - ((110 * 1000) + (timer.adjustment * 1000))) % adjustedCycleDurationMs;
